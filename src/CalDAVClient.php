...
class CalDAVClient
{
    ...
    function __construct($base_url, $user, $pass, $options = array())
    {
        ...
        curl_setopt_array($this->ch, array(
            ...
            CURLOPT_HTTPAUTH =>
            isset($options['auth']) ?  $options['auth'] : (CURLAUTH_BASIC | CURLAUTH_DIGEST),
    ...
    function FindPrincipal($url = null)
    {
        $xml = $this->DoPROPFINDRequest($url, array(
            'resourcetype', 'current-user-principal', 'owner', 'principal-URL',
...
EOXML;
        ...
        $this->requestMethod = "REPORT";
        $this->SetContentType("text/xml");
        if (isset($this->headers["match"])) {
            unset($this->headers["match"]);
        }
...
EOFILTER;
    ...
    {
        // WHEN UPDATING MULTIPLE VTODOS, REQUEST USES ETAG OF PREVIOUS VTODO
        // IN 'If-matches:' header!!!!!!!!!
        // THIS HAPPENS WHEN DoPUTRequest IS CALLED ONCE (calls SetMatch with ETAG)
        // THEN, FOR THE NEXT VTODO, IT CHECKS IF ETAG IS OK BY DOING A GET REQUEST WITH NEW URL
        // BUT SINCE this->headers WAS SET FOR OLD VTODO, THE NEW REQUEST WILL FAIL!!!
        // THIS IS A FIX FOR THAT
        if (isset($this->headers['match']) || !empty($this->headers['match'])) {
            unset($this->headers['match']);
        }
        //$href = str_replace( rawurlencode('/'),'/',rawurlencode($href));
            ...
            if (isset($this->xmltags['DAV::propstat'])) {
                foreach ($this->xmltags['DAV::propstat'] as $i => $node) {
                    ...
                    while ($this->xmlnodes[++$node]['level'] >= $level) {
                        if (
                            $this->xmlnodes[$node]['tag'] == 'DAV::status'
